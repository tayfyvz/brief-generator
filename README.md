...
- **LLM:** Claude structured outputs validated with Zod. `claude-sonnet-5` handles the high-volume calls (extraction, query planning, URL triage); `claude-opus-5` handles the three low-volume judgments that decide brief quality (entity resolution, verify, synthesis). Prompt prefixes are cached per run.
- **Hard rules:** every fact carries a `source_id` + verbatim quote (validated against the page snapshot; no quote, no fact); tier-4 sources (fan wikis, social media, forums) are citable only as clearly-labeled, low-confidence community data and are superseded by any tier 1-3 source with the same datum; unsupported facts are dropped by a fresh-context verifier; conflicts are surfaced with a tier+recency resolution note, never silently picked; facts older than ~18 months get a staleness badge.
...
| `ANTHROPIC_API_KEY` | no | stub | Claude (planning, extraction, verify, synthesis) |
| `OPENAI_API_KEY` / `OPENAI_MODEL` | no | stub / `gpt-4.1` | Fallback LLM when no Anthropic key is set |
| `TAVILY_API_KEY` | no | stub | Keyword web search (primary) |
| `EXA_API_KEY` | no | stub | Semantic search / find-similar (expansion only) |
| `FIRECRAWL_API_KEY` | no | stub | Page/PDF → markdown (falls back to direct fetch + pdf-parse) |
| `FIRECRAWL_RPM` | no | 10 | Firecrawl requests/minute budget |
| `GOOGLE_PLACES_API_KEY` | no | stub | Place ID → anchor (name/address/phone/website) |
| `MAX_ROUNDS` | no | 12 | Expansion round cap (runaway backstop; rounds stop on their own) |
| `MAX_SEARCHES_PER_RUN` / `MAX_FETCHES_PER_RUN` | no | 100 / 60 | Per-run budget; runs finish with what they have |
| `MAX_CONCURRENT_RUNS` / `DAILY_RUN_LIMIT` / `RATE_LIMIT_RUNS_PER_IP_PER_HOUR` | no | 3 / 25 / 5 | Public-endpoint abuse guards (cached reads uncapped) |
...
lib/graph       LangGraph state, nodes/ (one module per node, N0-N5), build, runner
lib/llm         Anthropic/OpenAI clients + offline stub + prompts/ (all prompt text)
...
lib/db          Drizzle schema, client, queries, migrations runner
